...
class Simulation_for_RL:
    ...
    def step(self, action):
        ...
        if (action): action = min(self.old_action + 1,n_floors-1) #go up
        else: action = max(self.old_action - 1,0) #go down
...
EPS_START = 1.0
EPS_END = 0.01 
EPS_LENGTH = int(MAX_TIMESTEPS*.75)
MAX_EPISODE_TIME = 400#00
load_params = False
use_softmax = False
...
folder = 'figures_num_people_long_up_down'
...
while steps_done < MAX_TIMESTEPS:
    ...
    if (episode_time > MAX_EPISODE_TIME):
        ...
        if (returns > max_return): 
            max_return = returns
            save(target_net, fm+'/'+param_file+'_max_return'+str(steps_done))
            print('New max return: ', max_return)
    ...
    if steps_done % 5000 == 0: # Save image of training every 1000 episodes
        print(state)
        with torch.no_grad():
