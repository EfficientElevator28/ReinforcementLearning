...
if (use_wait_time): state_size = n_floors*4 +n_floors + 1*2 + 1
else: state_size = state_size = n_floors*4 +n_floors + 1
...
param_file = sys.argv[3]#"1elevator_parameters"+str(n_floors)
...
def calc_cum_wait_time_per_floor(sim):
...
def modify_state(states, sim, action):
    states = np.array(states)

    one_hot_location_state = np.zeros((n_floors))
    one_hot_location_state[action] = 1
    
    waiting_states = calc_cum_wait_time_per_floor(sim)
    
    people_in_elevator = np.expand_dims(len(sim.building.elevators[0].riders), axis = 0)
        
    return np.concatenate((one_hot_location_state,states, people_in_elevator))#((states,one_hot_location_state,waiting_states))
...
def run_rl_v1_test():
    ...
    state = modify_state(state, sim, 0)
    print(state.shape)
    ...
    for i in range(tmp.shape[0]): # sets the initial state - doing it this way works around a bug
        state[i] = tmp[i]
    print(state, model.forward(state))
    ...
    while vis.alive == 1:
        ...
        if (action): action = min(elevator_location + 1,n_floors-1) #go up
        else: action = max(elevator_location - 1,0) #go down
        
        
        print("Action:",action)
        vals = model.forward(state).data.numpy()
        print("Values: ", vals,vals.argmax())
        state, reward, bld = sim.rl_step(starting_time=starting_time, action=action,
                                              person_scheduler=person_scheduler)   
        state = modify_state(state, sim, action)
        tmp = torch.tensor(state, device = device)
        state = torch.zeros(state_size) 
        for i in range(tmp.shape[0]): # sets the state - doing it this way works around a bug
            state[i] = tmp[i]
        elevator_location = action
